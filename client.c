       ...
       //while(1)
        {
            ...
            printf("\n Server Responce \n %s \n", buf);
            // 1 1546316624 1000 3  example line
...
void mailquery(char *query)
{
       ...
       // printf(" %s ",newmessage);
        ...
        for (i = 0; i < c-2; i++)
        {
            ...
            time_t     now;
            char timeStamp[80];
            strcpy(timeStamp,arr2[1]); //segmentation veriyor burada;
            struct tm  ts;
            char buffer[80];

            now = (time_t) atoi(timeStamp);

            ts = *localtime(&now);
            strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &ts);


            printf(" FROM %s ," ,arr2[3]); printf(" %s ,",buffer);printf(" %s BYTES",arr2[4]);
            printf("\n\n             %s \n",arr[1]);
...
void converter(void)
{
        ...
        for (i = 1; i < c-2; i++)
        {
            ...
            printf(" FROM %s ," ,arr2[2]); printf(" %s ,",buffer);printf(" %s BYTES",arr2[3]);
...
void header(char *title)
{
    ...
    printf("\n~~~~~~~~~~~~~~~~~~  2018/2019  ~~~~~~~~~~~~~~~~~~\n\n");
    printf("            ~ My Chat Server v0.1 is Ready. ~         \n\n");
